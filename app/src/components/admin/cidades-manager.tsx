...
export function CidadesManager({ initialCities }: { initialCities: City[] }) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setCities((prev) => [...prev, { id: `${prev.length + 1}`, name, state, slug: "", isActive: true }]);
    setName("");
    setState("");
  }
    ...
    <div className="flex flex-col gap-6">
      <form onSubmit={handleAdd} className="flex flex-wrap items-end gap-3 rounded-[var(--radius-md)] border border-border bg-surface p-4 card-shadow">
        ...
        <div>
          ...
          <input value={state} onChange={(e) => setState(e.target.value.toUpperCase())} maxLength={2} required className="input w-20" />
        </div>
